...
// field + minimum fill time silently rejects naive bots. No email verification in
// v3 (friction > value for a demo product — noted in the README/BACKLOG).
...
export async function signupAction(input: SignupFormInput): Promise<ActionResult<{ slug: string }>> {
  ...
  try {
    ...
    const { slug } = await signupClinic(payload, {
